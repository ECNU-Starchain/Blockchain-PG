	...
	if (completionTag)
	{
		...
		switch (queryDesc->operation)
		{
			...
			case CMD_INSERT:
                ...
                //added by gqx
                if(queryDesc->root_hash == NULL){
                    plan->roothash = NULL;
                }else{
                    plan->roothash = (char*)malloc(65);
                    strncpy(plan->roothash, queryDesc->root_hash, 32);
                    free(queryDesc->root_hash);
                }

                if(plan->hashinfo == NULL){
                    snprintf(completionTag, COMPLETION_TAG_BUFSIZE,
                       "INSERT %u %u", lastOid, queryDesc->estate->es_processed);
                }else{
                    snprintf(completionTag, COMPLETION_TAG_BUFSIZE,
                       "INSERT %u %u $%s", lastOid, queryDesc->estate->es_processed, plan->hashinfo);
                }
				break;
			case CMD_UPDATE:
				snprintf(completionTag, COMPLETION_TAG_BUFSIZE,
...
//    (*dest->sentRoot)("test", dest);
	...
	foreach(stmtlist_item, portal->stmts)
	{
			...
			((PlannedStmt *) stmt)->utilityStmt == NULL)
		{
			...
			PlannedStmt *pstmt = (PlannedStmt *) stmt;
            //added by gqx
            pstmt->hashinfo = portal->hashinfo;
			TRACE_POSTGRESQL_QUERY_EXECUTE_START();
            ...
            //added by gqx
            portal->roothash = pstmt->roothash;
