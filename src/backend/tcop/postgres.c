...
//added by gqx test vector 2020/4/3
#include <stdio.h>
#include <string.h>
#include <time.h>
...
static void parsestring(char *query, char *hashinfo);
...
pg_rewrite_query(Query *query)
{
    ...
    //added by gqx 2020/3/28 可验证查询直接跳过查询重写
    if (query->commandType == CMD_UTILITY || query->isAuthtype == TRUE)
	{
 ...
 */
static void
exec_simple_query(const char *query_string, char *tmp_hash)
{
    ...
    //added by dsj
    char       command_str[32];
    memset( command_str,0,32);
    bool isAuthinsert=false;
	/*
...
//added by dsj
         ...
         * we are paparsestring(query_string);ssing here is in MessageContext, which will outlive the
		 * portal anyway.
		 */
...
//        }
        ...
        if(portal->roothash != NULL){
            hashinfo = (char*)malloc(65);
            memset(hashinfo, 0, 65);
            strncpy(hashinfo, portal->roothash, 32);
            free(portal->hashinfo);
	    isAuthinsert=true;
        }
   ...
   if(isAuthinsert){
    parsestring(query_string, hashinfo);
   }  
	/*
...
static void parsestring(char *query_string, char *hashinfo){
        //added by dsj
        char* str_create=(char*)malloc(13);
        memset(str_create,0 , 13);
        strcpy(str_create,"CREATE AUTHTABLE");
        char* str_insert=(char*)malloc(32);
        memset(str_insert, 0, 6);
        strcpy(str_insert,"INSERT");
        int k = 0;
        int state1 = -1, state2 = -1;
        for(k =0; k < strlen(str_create); k++){
            if(query_string[k] == str_create[k] || query_string[k] == str_create[k] + 32){
                state1 = 0;
            }else{
                state1 = -1;
                break;
        ...
        for(k =0; k < strlen(str_insert); k++){
            if(query_string[k] == str_insert[k] || query_string[k] == str_insert[k] + 32){
                state2 = 1;
            }else{
                state2 = -1;
                break;
        ...
        if(state1 == 0){
    //        const char *query_string=(const char*)malloc(32);
    //        strcpy(query_string,"CREATE TABLE userinfo(name VARCHAR(20), age INT, signup_date DATE);");
            char *arZe_1=(char*)malloc(32);
            char *arZe_2 = (char*)malloc(32);
            strcpy(arZe_1," ");
            strcpy(arZe_2,"(");
    //        char *arZe_1 = " ";
    //        char *arZe_2 = "(";
            char *arGet=(char*)malloc(32);
            char *szGet = (char*)malloc(32);
            int count=0;
            for(szGet = strtok(query_string, arZe_1); szGet != NULL; szGet = strtok(NULL, arZe_1))
            {
                strcpy(arGet, szGet);
                count++;
                if(count==3){
            ...
            //free(arZe_1);
            szGet = strtok(arGet, arZe_2);
            //free(arZe_2);
            //free(arGet);
            if(strcmp(szGet,"TABLE")!=0&&strcmp(szGet,"table")!=0){
                if(!strstr(szGet,"history")){
                    char *firstStr="CREATE TABLE ";
                    char *lastStr="_history(tx_id VARCHAR(20),operation_type VARCHAR(20),operation_content TEXT,signature TEXT,operator VARCHAR(20),state_root VARCHAR(32),previsou_hash TEXT,hash TEXT,date DATE);";
    //                char *lastStr="_history(tx_id VARCHAR(20),operation_type VARCHAR(20));";
                    const char *create_string = (const char *) malloc(strlen(firstStr) + strlen(szGet)+ strlen(lastStr));
                    sprintf(create_string, "%s%s%s", firstStr,szGet,lastStr);
                    printf("%s\n", create_string);
                    exec_simple_query(create_string, NULL);
                    free(create_string);
                    //free(create_string);
                }
            }
            //free(szGet);
    //        const char *create_string=(const char*)malloc(1024);
    //        strcat(create_string,"CREATE TABLE ");
    //        strcat(create_string,szGet);
    //        strcat(create_string,"_history(tx_id VARCHAR(20),operation_type VARCHAR(20),operation_context VARCHAR(20),signature VARCHAR(20),state_root VARCHAR(20),hash VARCHAR(20),create_date DATE);");

        }else if(state2==1){

            if(hashinfo == NULL || strlen(hashinfo) == 0)
                return;

            char *strGet_1 = (char*)malloc(32);
            memset(strGet_1,0,32);
            char *strGet_2 = (char*)malloc(32);
            memset(strGet_2,0,32);
            char *strGet_3 = (char*)malloc(32);
            memset(strGet_3,0,32);
            char *strGet_4 = (char*)malloc(32);
            memset(strGet_4,0,32);
            char *str_query_1 = (char*)malloc(256);
            memset(str_query_1 ,0,256);
            char *str_query_2 = (char*)malloc(256);
            memset(str_query_2,0,256);
            char *str_query_3 = (char*)malloc(256);
            memset(str_query_3,0,256);
            char *str_query_4 = (char*)malloc(256);
            memset(str_query_4,0,256);
            int count_str=0;
            for(strGet_1 = strtok(query_string, "("); strGet_1 != NULL; strGet_1 = strtok(NULL, "("))
            {
                if(strstr(strGet_1,"VALUES")||strstr(strGet_1,"values")){
                    count_str++;
                }
                if(count_str==1){
                    strcpy(str_query_1,strGet_1);
                }
            }
            for(strGet_2 = strtok(str_query_1, ")"); strGet_2  != NULL; strGet_2  = strtok(NULL, ")"))
            {
                if(strcmp(strGet_2,";")!=0&&strcmp(strGet_2,"; ")!=0&&strcmp(strGet_2," ;")!=0){
                    strcpy(str_query_2,strGet_2);
                }
            }
            for(strGet_3 = strtok(str_query_2, "'"); strGet_3 != NULL; strGet_3 = strtok(NULL, "'"))
            {
                sprintf(str_query_3, "%s%s",str_query_3,strGet_3);

            }
            for(strGet_4= strtok(str_query_3, " "); strGet_4 != NULL; strGet_4 = strtok(NULL, " "))
            {
                sprintf(str_query_4, "%s%s",str_query_4,strGet_4);
            }
            printf("%s\n",str_query_4);
            char *Str="'" ;
            char *str_query_5=(char*)malloc(strlen(Str)+ strlen(str_query_4)+ strlen(Str));
            sprintf(str_query_5, "%s%s%s", Str,str_query_4,Str);
    ...
    //data:
            ...
            sprintf(str_time,"%d%s%d%s%d",1900+p->tm_year,"-",1+p->tm_mon,"-",p->tm_mday);
            printf("%s\n",str_time);
            char *str_query_6=(char*)malloc(strlen(Str)+ strlen(str_time)+ strlen(Str));
            sprintf(str_query_6, "%s%s%s", Str,str_time,Str);
    ////tx_id:
              char *str_optype=(char*)malloc(33);
              memset(str_optype,0,33);
              strcpy(str_optype,"1");
              char *str_query_10=(char*)malloc(strlen(Str)+ strlen(str_optype)+ strlen(Str));
              sprintf(str_query_10, "%s%s%s", Str,str_optype,Str);
    //operator:
              char *str_opter=(char*)malloc(33);
              memset(str_opter,0,33);
              strcpy(str_opter,"dsj");
              char *str_query_11=(char*)malloc(strlen(Str)+ strlen(str_opter)+ strlen(Str));
              sprintf(str_query_11, "%s%s%s", Str,str_opter,Str);
    //state_root:
              char *str_root=(char*)malloc(33);
              memset(str_root,0,33);
              strncpy(str_root,hashinfo,32);
              char *str_query_13=(char*)malloc(strlen(Str)+ strlen(str_root)+ strlen(Str));
              sprintf(str_query_13, "%s%s%s", Str,str_root,Str);
    //signature :
              char *str_sig=(char*)malloc(33);
              memset(str_sig,0,33);
              sha256_pg(str_query_5,str_sig);
              char *str_query_12=(char*)malloc(strlen(Str)+ strlen(str_sig)+ strlen(Str));
              sprintf(str_query_12, "%s%s%s", Str,str_sig,Str);
    //previsou_hash:
            char *str_phash=(char*)malloc(33);
            memset(str_phash,0,33);
            //strcpy(str_phash,"11111111111111111111111111111111");
            strcpy(str_phash,previsou_hash);
            char *str_query_8=(char*)malloc(strlen(Str)+ strlen(str_phash)+ strlen(Str));
            sprintf(str_query_8, "%s%s%s", Str,str_phash,Str);
    //hash:
            char *str_half_hash=(char*)malloc(33);
            memset(str_half_hash,0,33);
            sha256_pg(str_query_5,str_half_hash);
            char* tmp_hash=(char*)malloc(64);
            memset(tmp_hash,0,64);
            sprintf(tmp_hash,"%s%s",previsou_hash,str_half_hash);
            char *str_hash=(char*)malloc(33);
            memset(str_hash,0,33);
            sha256_pg(tmp_hash,str_hash);
            /*strcpy(previsou_hash,str_hash)*/;
            char *str_query_7=(char*)malloc(strlen(Str)+ strlen(str_hash)+ strlen(Str));
            sprintf(str_query_7, "%s%s%s", Str,str_hash,Str);
            char *arZe_3=(char*)malloc(32);
            char *arZe_4 = (char*)malloc(32);
            strcpy(arZe_3," ");
            strcpy(arZe_4,"(");
            char *arGet1=(char*)malloc(32);
            char *szGet1 = (char*)malloc(32);
            int count=0;
            for(szGet1 = strtok(query_string, arZe_3); szGet1 != NULL; szGet1 = strtok(NULL, arZe_3))
            {
                strcpy(arGet1, szGet1);
                count++;
                if(count==3){
                    break;
                }
            ...
            char *my_hash = (char*)malloc(65);
            memset(my_hash, 0, 65);
            strncpy(my_hash, hashinfo, 32);
            strncpy(my_hash + 32, str_hash, 32);
            free(hashinfo);


            szGet1 = strtok(arGet1, arZe_4);
            if(strcmp(szGet1,"TABLE")!=0&&strcmp(szGet1,"table")!=0){
                if(!strstr(szGet1,"history")&&!strstr(szGet1,"pg_")){
                    strcpy(previsou_hash,str_hash);
                    //tx_id:
                    char *str_txid=(char*)malloc(33);
                    memset(str_txid,0,33);
                    strcpy(str_txid,tx_id);
                    int tmp_id=atoi(tx_id);
                    tmp_id++;
                    sprintf(tx_id,"%d",tmp_id);
                    char *str_query_9=(char*)malloc(strlen(Str)+ strlen(str_txid)+ strlen(Str));
                    sprintf(str_query_9, "%s%s%s", Str,str_txid,Str);
                    char *firstStr="INSERT INTO ";
                    //char *nextStr="_history(tx_id, operation_type, operation_content, signature, operator, state_root, previsou_hash, hash, date) VALUES('0', '0', ";
                    char *nextStr="_history(tx_id, operation_type, operation_content, signature, operator, state_root, previsou_hash, hash, date) VALUES(";
                    char *nextStr2=", ";
                   // char *nextStr1=", '0', '0',";
                    //char *nextStr1="'0', '0',";
                    char *nextStr1 = (char *) malloc(100);
                    sprintf(nextStr1, "%s%s%s%s", str_query_12,nextStr2,str_query_11,nextStr2);
                    char *endStr=");";
                    char *insert_string = (char *) malloc(1000);
                    strcpy(insert_string,firstStr);
                    strcat(insert_string,szGet1);
                    strcat(insert_string,nextStr);
                    strcat(insert_string,str_query_9);
                    strcat(insert_string,nextStr2);
                    strcat(insert_string,str_query_10);
                    strcat(insert_string,nextStr2);
                    strcat(insert_string,str_query_5);
                    strcat(insert_string,nextStr2);
                    strcat(insert_string,nextStr1);
                    strcat(insert_string,str_query_13);
                    strcat(insert_string,nextStr2);
                    strcat(insert_string,str_query_8);
                    strcat(insert_string,nextStr2);
                    strcat(insert_string,str_query_7);
                    strcat(insert_string,nextStr2);
                    strcat(insert_string,str_query_6);
                    strcat(insert_string,endStr);
//                    printf("%s\n", insert_string);
                    exec_simple_query(insert_string, my_hash);
                    free(strGet_1);
                    free(strGet_2);
                    free(strGet_3);
                    free(strGet_4);
...
//added by dsj
    previsou_hash=(char*)malloc(32);
    memset(previsou_hash,0,32);
    strcpy(previsou_hash,"11111111111111111111111111111111");

    tx_id=(char*)malloc(32);
    memset(tx_id,0,32);
    strcpy(tx_id,"1");
