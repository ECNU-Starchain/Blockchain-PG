...
transformAExprOp(ParseState *pstate, A_Expr *a)
{
	...
	else
	{
        ...
        //added by gqx 2020/3/29
        if(pstate->isAuthType){
            Auth_Expr *authexpr = makeNode(Auth_Expr);
                authexpr->type = T_Auth_Expr;
                authexpr->lexpr = lexpr;
                authexpr->rexpr = rexpr;
                authexpr->opName = strVal(linitial(a->name));
                return (Node *)authexpr;
        }else{
...
transformAExprAnd(ParseState *pstate, A_Expr *a)
{
    ...
    //added by gqx 2020/3/29
    if(pstate->isAuthType){
        Auth_Op_Expr *authexpr = makeNode(Auth_Op_Expr);
        authexpr->type = T_Auth_Op_Expr;
        authexpr->lexpr = lexpr;
        authexpr->rexpr = rexpr;
        authexpr->authType = 1;
        return (Node *)authexpr;
