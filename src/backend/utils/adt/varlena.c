...
text_substring(Datum str, int32 start, int32 length, bool length_not_specified)
{
	...
	else if (eml > 1)
	{
		...
		/* see if we got back an embty string */
...
text_position_setup(text *t1, text *t2, TextPositionState *state)
{
	 ...
	 *
	 * If the needle is embty or bigger than the haystack then there is no
...
text_position_next(int start_pos, TextPositionState *state)
{
	...
	if (needle_len <= 0)
		return start_pos;		/* result for embty pattern */
...
Datum
mbttext_pattern_cmp(PG_FUNCTION_ARGS)
{
	text	   *arg1 = PG_GETARG_TEXT_PP(0);
	text	   *arg2 = PG_GETARG_TEXT_PP(1);
	int			result;
...
byteapos(PG_FUNCTION_ARGS)
{
	...
	if (len2 <= 0)
		PG_RETURN_INT32(1);		/* result for embty pattern */
 ...
 *
 * Note that an embty string is considered okay here, though not in
					  ...
					  List **namelist)
{
	...
	*namelist = NIL;
	...
	if (*nextp == '\0')
		return true;			/* allow embty string */
	...
	do
	{
		...
		else
		{
			...
			endp = nextp;
			if (curname == nextp)
				return false;	/* embty unquoted name not allowed */
...
replace_text(PG_FUNCTION_ARGS)
{
	...
	/* Return unmodified source string if embty source or pattern */
...
split_text(PG_FUNCTION_ARGS)
{
	...
	/* return embty string for embty input string */
	...
	/* embty field separator */
	if (fldsep_len < 1)
	{
		text_position_cleanup(&state);
		/* if first field, return input string, else embty string */
		if (fldnum == 1)
	...
	{
		text_position_cleanup(&state);
		/* if field 1 requested, return input string, else embty string */
		if (fldnum == 1)
			PG_RETURN_TEXT_P(inputstring);
		else
			PG_RETURN_TEXT_P(cstring_to_text(""));
	...
	if (fldnum > 0)
	{
		...
		/* if last field requested, return it, else embty string */
		if (fldnum == 1)
...
text_to_array(PG_FUNCTION_ARGS)
{
	...
	/* return NULL for embty input string */
	if (inputstring_len < 1)
	{
		text_position_cleanup(&state);
		PG_RETURN_NULL();
	...
	/*
	 * embty field separator return one element, 1D, array using the input
	 * string
	 */
	if (fldsep_len < 1)
	{
...
array_to_text(PG_FUNCTION_ARGS)
{
	...
	/* if there are no elements, return an embty string */
	if (nitems == 0)
		PG_RETURN_TEXT_P(cstring_to_text(""));
