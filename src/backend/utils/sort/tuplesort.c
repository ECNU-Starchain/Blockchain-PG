 ...
 * next tuple from its source tape (if any).  When the heap embties, the merge
 ...
 *
 *
 * NOTES about memory consumbtion calculations:
 *
...
static int comparetup_index_btree(const SortTuple *a, const SortTuple *b,
					   Tuplesortstate *state);
static int comparetup_index_mbtree(const SortTuple *a, const SortTuple *b,
					   Tuplesortstate *state);					   
...
static void reversedirection_index_mbtree(Tuplesortstate *state);
...
//改动
Tuplesortstate *
tuplesort_begin_index_mbt(Relation indexRel,
							bool enforceUnique,
							int workMem, bool randomAccess)
{
	Tuplesortstate *state = tuplesort_begin_common(workMem, randomAccess);
	...
	state->comparetup = comparetup_index_mbtree;
	state->copytup = copytup_index;
	state->writetup = writetup_index;
	state->readtup = readtup_index;
	state->reversedirection = reversedirection_index_mbtree;

	state->indexRel = indexRel;
	state->indexScanKey = _mbt_mkscankey_nodata(indexRel);
	state->enforceUnique = enforceUnique;
 ...
 * pointing to garbage.  Be careful not to attembt to use or free such
 * pointers afterwards!
 */
void
...
grow_memtuples(Tuplesortstate *state)
{
	 ...
	 * assumbtion should be good.  But let's check it.)
	 */
		...
		case TSS_SORTEDONTAPE:
			...
			*should_free = true;
			...
			if (state->eof_reached)
			{
				 ...
				 * word.  If seek fails we must have a completely embty file.
				 */
...
inittapes(Tuplesortstate *state)
{
	...
	state->memtupcount = 0;		/* make the heap embty */
	for (j = 0; j < ntuples; j++)
	{
 ...
 * already been written to initial runs on tape (see dumbtuples).
 */
static void
mergeruns(Tuplesortstate *state)
{
	...
	{
		/*
		 * At this point we know that tape[T] is embty.  If there's just one
		...
		/* Step D5: merge runs onto tape[T] until tape[P] is embty */
...
mergeonerun(Tuplesortstate *state)
{
	...
	/*
	 * When the heap embties, we're done.  Write an end-of-run marker on the
...
/*
 * dumbtuples - remove tuples from heap and write to tape
 *
 ...
 *
 * If we embty the heap, close out the current run and return (this should
...
dumptuples(Tuplesortstate *state, bool alltuples)
{
		   ...
		   state->memtupcount >= state->memtupsize)
	{
		...
		/*
		 * If the heap is embty *or* top run number has changed, we've
			...
			state->currentRun != state->memtuples[0].tupindex)
		{
			...
			/*
			 * Done if heap is embty, else prepare for new run.
			 */
			if (state->memtupcount == 0)
				break;
...
sort_bounded_heap(Tuplesortstate *state)
{
	 ...
	 * entry, which we can prombtly store in the newly freed slot at the end.
...
/*
 * Insert a new tuple into an embty or existing heap, maintaining the
...
//改动
static int
comparetup_index_mbtree(const SortTuple *a, const SortTuple *b,
					   Tuplesortstate *state)
{
	/*
	 * This is similar to _bt_tuplecompare(), but we have already done the
...
static void
reversedirection_index_mbtree(Tuplesortstate *state)
{
	ScanKey		scanKey = state->indexScanKey;
	int			nkey;
	...
	for (nkey = 0; nkey < state->nKeys; nkey++, scanKey++)
	{
		scanKey->sk_flags ^= (SK_mbt_DESC | SK_mbt_NULLS_FIRST);
	}
}
static void
