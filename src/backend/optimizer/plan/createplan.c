...
static AuthIndexScan *create_authindexscan_plan(PlannerInfo *root, AuthIndexPath *best_path,
                    List *tlist, List *scan_clauses);

static AuthIndexScan *make_authindexscan(List *qptlist,
                                         List *qpqual,
                                         Index scanrelid,
...
create_scan_plan(PlannerInfo *root, Path *best_path)
{
	...
	switch (best_path->pathtype)
	{
        ...
        case T_AuthIndexScan:
            plan = (Plan *) create_authindexscan_plan(root,
                                                  (AuthIndexPath *) best_path,
                                                  tlist,
                                                  scan_clauses);
            break;
...
//added by gqx 2020/3/30
static AuthIndexScan *
create_authindexscan_plan(PlannerInfo *root, AuthIndexPath *best_path,
                    List *tlist, List *scan_clauses){

    AuthIndexScan* auth_plan;
    ...
    //暂时不做任何处理
    auth_plan = make_authindexscan(tlist,
                                   best_path->indexquals,
                                   scan_relid,
                                   best_path->indexscandir);
    if(best_path->indexoids != NIL){
        ListCell* indexId;
        foreach(indexId, best_path->indexoids){
            Oid			indexoid = lfirst_oid(indexId);
            auth_plan->indexid = indexoid;
        }
    }

    copy_path_costsize(&auth_plan->scan.plan, &best_path->path);
    return auth_plan;
}
...
//added by gqx 2020/3/30
static AuthIndexScan *
make_authindexscan(List *qptlist,
             List *qpqual,
             Index scanrelid,
             ScanDirection indexscandir)
{
    AuthIndexScan    *node = makeNode(AuthIndexScan);
    Plan	   *plan = &node->scan.plan;
