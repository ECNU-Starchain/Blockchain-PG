...
//added by gqx 2020/3/28
//create_auth_index_path
AuthIndexPath *
create_auth_index_path(PlannerInfo *root,
                            ScanDirection indexscandir,
                            RelOptInfo *rel){

    AuthIndexPath  *pathnode = makeNode(AuthIndexPath);

    pathnode->path.pathtype = T_AuthIndexScan;
    pathnode->path.parent = rel;
    pathnode->indexscandir = indexscandir;
    pathnode->indexquals = root->parse->authquals;
    List	   *result = NIL;
    /*
     * added by gqx 2020/3/30
     * 对于可验证查询只能选择一条路径，此时将cost都默认设置为0
     */
    pathnode->indextotalcost = 0.0;
    pathnode->path.startup_cost = 0.0;
    pathnode->path.total_cost = 0.0;
    ListCell   *ilist;
    foreach(ilist, rel->indexlist)
    {
        IndexOptInfo *index = (IndexOptInfo *) lfirst(ilist);
        result = lappend_oid(result, index->indexoid);
    }
    pathnode->indexoids = result;
