...
//added by gqx 2020/3/31
static TupleTableSlot *AuthIndexNext(AuthIndexScanState *node);
...
/* ----------------------------------------------------------------
 *		ExecAuthIndexScan(node)
 * ----------------------------------------------------------------
 */

//added by gqx
static int i_times = 1;
static bool flag = true;
static int i_time = 0;
static int j_time = 0;
static pgvo myvo = NULL;
//by zcy modified 2020/6/30
static char* vo_str;
static char* hashpmap_str;
//modified end	


//pgvector vectors_data;
static TupleTableSlot *AuthIndexNext(AuthIndexScanState *node){
    HeapTuple	tuple;
    IndexScanDesc scandesc;
    Index		scanrelid;
     ...
     */
    scandesc = node->iss_ScanDesc; //暂时这样，因为没有索引
    slot = node->ss.ss_ScanTupleSlot;
    if(flag){
//        starttime=GetTime();
		//by zcy modified 2020/6/30
        ...
        Auth_Expr* authexpr = (Auth_Expr*)node->ss.ps.plan->qual;
        int left, right;
        if(authexpr->type == T_Auth_Op_Expr){
            Auth_Expr* lexpr = (Auth_Expr*)authexpr->lexpr;
            Auth_Expr* rexpr = (Auth_Expr*)authexpr->rexpr;
            Const v1 = *(Const*)lexpr->rexpr;
            Const v2 = *(Const*)rexpr->rexpr;
            if(strcmp(lexpr->opName, "<") && strcmp(rexpr->opName, ">")){
                left = v1.constvalue;
                right = v2.constvalue;
            }
            if(strcmp(lexpr->opName, ">") && strcmp(rexpr->opName, "<")){
            ...
            Const v = *(Const*)authexpr->rexpr;
            if(strcmp(authexpr->opName, ">")){
                left = v.constvalue;
            }
            if(strcmp(authexpr->opName, "<")){
                right = v.constvalue;
		...
		//by zcy modified 2020/6/30
//        starttime=GetTime();
		myvo = auth_index_getnext(scandesc, indexRelation, left, right);
//        endtime=GetTime();
...
//        }
        ...
        vo_str = (char*)malloc(myvo->mbtree_size*DEFAULTSIZE*sizeof(char));
		hashpmap_str = (char*)malloc(myvo->mbtree_size*DEFAULTSIZE*sizeof(char));
        MemSet(vo_str, 0, myvo->mbtree_size*DEFAULTSIZE);
        MemSet(hashpmap_str, 0, myvo->mbtree_size*DEFAULTSIZE);
//        starttime=GetTime();
        vo_serialize(myvo, vo_str);
//        endtime=GetTime();
//        mytime=mytime + ((double)(endtime-starttime));
//        count1++;
//        if(count1==99){
...
//           mytime=0;
//        }
        for(j = 0; j < myvo->mbtree_size*DEFAULTSIZE; j++){
            hashpmap_str[j] = myvo->hashmap[j];
        }
//                endtime=GetTime();
//                mytime=mytime + ((double)(endtime-starttime));
//                count1++;
...
//                }
       ...
       rangevo_init(vo2,myvo->mbtree_size*DEFAULTSIZE);
       vo_deserialize(vo2, vo_str);
       Hashmap* hashp2[BPLUS_MAX_LEVEL];
       for(i = 0; i < BPLUS_MAX_LEVEL; i++){
           hashp2[i] = hashmap_new(NUM_BUCKETS, myhash, compare);
       }
       hashmap_deserialize(hashp2, hashpmap_str);
       char* root = (char*)malloc(33);
       //reconstruct_root(tree,hashp,vo);
...
*/
    ...
    if(!flag){
        if(myvo->itemdata.size == vector_index){
//            count1++;
//            endtime=GetTime();
//            mytime=mytime + ((double)(endtime-starttime));
//            count1++;
...
//            }
//            starttime=GetTime();
//            vector_destroy(&myvo->itemdata)>
            //by zcy modified 2020/6/30
            slot->hashmap_num = myvo->mbtree_size;
            //modified end
            tuple = NULL;
            flag = true;
            //by zcy modified 2020/6/30
            slot->hashmap_str = (char*)malloc(myvo->mbtree_size*DEFAULTSIZE*sizeof(char));
            slot->vo_str = (char*)malloc(myvo->mbtree_size*DEFAULTSIZE*sizeof(char));
            slot->flag = true;
            memset(slot->hashmap_str, 0, myvo->mbtree_size*DEFAULTSIZE);
            memset(slot->vo_str, 0, myvo->mbtree_size*DEFAULTSIZE);
            for(j = 0; j < myvo->mbtree_size*DEFAULTSIZE; j++){
                slot->hashmap_str[j] = hashpmap_str[j];
                slot->vo_str[j] = vo_str[j];
            }
            //modified end
//            endtime=GetTime();
//            mytime=mytime + ((double)(endtime-starttime));
...
//            pgvo vo2 =(rangevo*)malloc(sizeof(rangevo));
//            rangevo_init(vo2);
//            vo_deserialize(vo2, slot->vo_str);
//            Hashmap* hashp2[BPLUS_MAX_LEVEL];
//            for(i = 0; i < BPLUS_MAX_LEVEL; i++){
//                hashp2[i] = hashmap_new(NUM_BUCKETS, myhash, compare);
//            }
//            hashmap_deserialize(hashp2, slot->hashmap_str);
...
//            printf("--------");
        ...
        if(myvo->itemdata.size > vector_index){
//            starttime=GetTime();
            ItemPointer tid= *(ItemPointer*)vector_get(&myvo->itemdata, vector_index);
            vector_index++;
            tuple = (HeapTuple)palloc(sizeof(HeapTupleData));
            Relation relation = node->ss.ss_currentRelation;
            scandesc->xs_cbuf = ReadBuffer(relation, ItemPointerGetBlockNumber(tid));
            Buffer buffer = scandesc->xs_cbuf;
...
//        starttime=GetTime();
        ...
        if(myvo != NULL){
			//by zcy modified 2020/6/30
            vector_destroy(&myvo->itemdata);
			if(vector_is_empty(&myvo->itemdata) == false)
			{
				vector_clear(&myvo->itemdata);
			}
			if(vector_is_empty(&myvo->data) == false)
			{
				vector_clear(&myvo->data);
			}
			if(myvo->hashmap != NULL)
			{
				free(myvo->hashmap);
				myvo->hashmap = NULL;
			}
...
TupleTableSlot *
ExecAuthIndexScan(AuthIndexScanState *node){
    /*
     * use IndexNext as access method
     */
    return ExecScan(&node->ss, (ExecScanAccessMtd) AuthIndexNext);
}
 ...
 */
void
ExecEndAuthIndexScan(AuthIndexScanState *node)
{
     ...
     */
    if (indexScanDesc && BufferIsValid(indexScanDesc->xs_cbuf)){
        ReleaseBuffer(indexScanDesc->xs_cbuf);
    }

    /*
     * close the heap relation.
     */
    indexScanDesc->xs_ctup.t_data = NULL;
    indexScanDesc->xs_cbuf = InvalidBuffer;
    indexScanDesc->xs_hot_dead = false;
    indexScanDesc->xs_next_hot = InvalidOffsetNumber;
//    Assert(indexScanDesc->indexRelation->rd_refcnt > 0);
//    indexScanDesc->indexRelation->rd_refcnt -= 1;
//    IndexScanEnd(indexScanDesc);
//    if (indexScanDesc)
...
//added by gqx 2020/3/20
AuthIndexScanState *
ExecInitAuthSeqScan(AuthIndexScan *node, EState *estate, int eflags){
    AuthIndexScanState *authscanstate;
    Relation	currentRelation;
    bool		relistarget = true;

    /*
     * create state structure
     */
    authscanstate = makeNode(AuthIndexScanState);
    authscanstate->ss.ps.plan = (Plan *) node;
    authscanstate->ss.ps.state = estate;

    authscanstate->iss_RelationDesc = index_open(node->indexid,
                                     relistarget ? NoLock : AccessShareLock);
     ...
     * create expression context for node
     */
    ExecAssignExprContext(estate, &authscanstate->ss.ps);

    authscanstate->ss.ps.ps_TupFromTlist = false;

    //初始化投影运算
    authscanstate->ss.ps.targetlist = (List *)
        ExecInitExpr((Expr *) node->scan.plan.targetlist,
                     (PlanState *) authscanstate);
    authscanstate->ss.ps.qual = node->indexqual;

#define AUTHINDEXSCAN_NSLOTS 2

    /*
     * tuple table initialization
     */
    ExecInitResultTupleSlot(estate, &authscanstate->ss.ps);
    ExecInitScanTupleSlot(estate, &authscanstate->ss);
    ...
    authscanstate->ss.ss_currentRelation = currentRelation;
    authscanstate->ss.ss_currentScanDesc = NULL;
    /*
     * get the scan type from the relation descriptor.
     */
    ExecAssignScanType(&authscanstate->ss, RelationGetDescr(currentRelation));
    /*
     * Initialize result tuple type and projection info.
     */
    ExecAssignResultTypeFromTL(&authscanstate->ss.ps);
    ExecAssignScanProjectionInfo(&authscanstate->ss);//p *(TargetEntry *)scan->plan.targetlist->head->data.ptr_value 获取投影的值

    //relistarget = ExecRelationIsTargetRelation(estate, node->scan.scanrelid);
    //使用indexid获取索引的RelationData结构存放于iss_RelationDesc字段中。
    //authscanstate->iss_RelationDesc = index_open(node->indexid,relistarget ? NoLock : AccessShareLock);
    ...
    authscanstate->iss_ScanDesc = index_beginscan(currentRelation,
                                               authscanstate->iss_RelationDesc,
                                               estate->es_snapshot,
                                               NULL,
                                               NULL);

    return authscanstate;
}
...
//added by gqx
int
ExecCountSlotsAuthIndexScan(IndexScan *node)
{
    return ExecCountSlotsNode(outerPlan((Plan *) node)) +
        ExecCountSlotsNode(innerPlan((Plan *) node)) + AUTHINDEXSCAN_NSLOTS;
}
