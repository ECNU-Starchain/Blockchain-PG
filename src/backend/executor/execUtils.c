 ...
 * ----------------------------------------------------------------
 */
static pg_index_info* getTupleValue(TupleTableSlot *slot,
                           DestReceiver *dest, int indkey){
    TupleDesc	typeinfo = slot->tts_tupleDescriptor;
    DR_printtup *myState = (DR_printtup *) dest;
    StringInfoData buf;
    int			natts = typeinfo->natts;
    int			i;
    pg_index_info* result = (pg_index_info*)malloc(sizeof(pg_index_info));
    result->str = (char*)malloc(sizeof(char)*64);
    MemSet(result->str, 0, 64);
    //result->str = (char*)malloc(sizeof(char)*64);
   // MemSet(result->str, 0, 64);
                      ...
                      DestReceiver *dest)
{
	...
	for (i = 0; i < numIndices; i++)
	{
        ...
        if(indexInfo->isAuthType == true){
            int indkey = indexInfo->ii_NumIndexAttrs;
            pg_index_info* rs = getTupleValue(slot, dest, indkey);
            mbt_index_insert(relationDescs[i], rs, tupleid, heapRelation);
            // (*dest->sentRoot)(rs->root_str, dest);
            estate->root_hash = (char*)malloc(33);
			
            // strncpy(estate->root_hash, rs->root_str, 33);
			 strcpy(estate->root_hash, "test");
	    free(rs->str);
	    //free(rs->root_str);
	    free(rs);
            continue;
