...
//just test added by gqx

#define FILE_PATH "/home/gqx/test.txt"  // 文件路径
void gqx_test(){
    char name[20] = "限制具体数目";  // 姓名
    int age = 22;  // 年龄
    int studNo = 12;  // 学号
    float score = 11.0;  // 平均分
    FILE *fp;  // 文件指针
					 ...
					 ScanDirection direction, long count)
{
    ...
    //added by gqx
    queryDesc->root_hash = estate->root_hash;
   // free(estate->root_hash);
//    (*dest->printVO) ("6b88c095df6b2a70e3" ,dest);
...
//       }
    ...
    if(planstate->type == T_AuthIndexScanState && planSlot->flag == true ){
		//by zcy modified 2020/6/30
		int count = 0;
		for(count = 0; count < slot->hashmap_num; count++)
		{
			(*dest->printVO) (slot ,dest,count);
		}
		//modified end	
    }
...
//    gqx_test();
    ...
    if (resultRelInfo->ri_NumIndices > 0)
        ExecInsertIndexTuples(slot, &(tuple->t_self), estate, false, dest);
...
lreplace:;
	...
	if (resultRelInfo->ri_NumIndices > 0 && !HeapTupleIsHeapOnly(tuple))
        ExecInsertIndexTuples(slot, &(tuple->t_self), estate, false, NULL);
