...
bool mbt_index_insert(Relation indexRelation, pg_index_info *values,
             ItemPointer heap_t_ctid,
             Relation heapRelation){
    FmgrInfo   *procedure;
     ...
     * have the am's insert proc do all the work.
     */
    return DatumGetBool(FunctionCall4(procedure,
                                      PointerGetDatum(indexRelation),
                                      PointerGetDatum(values),
                                      PointerGetDatum(heap_t_ctid),
                                      PointerGetDatum(heapRelation)));
...
static IndexScanDesc
index_auth_beginscan_internal(Relation indexRelation,
                         int nkeys, ScanKey key)
{
    IndexScanDesc scan;
    FmgrInfo   *procedure;
...
index_getnext(IndexScanDesc scan, ScanDirection direction)
{
	...
	for (;;)
	{
		...
		for (;;)
		{
             ...
             * the tuples in the HOT cRelation indexRelationhain are dead, we'll signal the index AM
...
mypgvo auth_index_getnext(IndexScanDesc scan, Relation indexRelation, int left, int right){
    HeapTuple	heapTuple = &scan->xs_ctup;
    ItemPointer tid = &heapTuple->t_self;
    FmgrInfo   *procedure;
    ...
    Assert(TransactionIdIsValid(RecentGlobalXmin));
    mypgvo myvo = DatumGetPointer(FunctionCall4(procedure,
                                       PointerGetDatum(scan),
                                       PointerGetDatum(indexRelation),
                                       Int32GetDatum(left),
                                       Int32GetDatum(right)));
