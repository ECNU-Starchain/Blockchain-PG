...
static void printVO(TupleTableSlot *slot, DestReceiver *self,int sequence_num);
//modified end	
static void sentRoot(char *root, DestReceiver *self);
...
printtup_create_DR(CommandDest dest)
{
    ...
    //added by gqx
    self->pub.printVO = printVO;
    self->pub.sentRoot = sentRoot;
...
//by zcy modified 2020/6/30
static void
printVO(TupleTableSlot *slot, DestReceiver *self,int sequence_num)
{
//modified end	
    DR_printtup *myState = (DR_printtup *) self;
    StringInfoData buf;
    int natts = 2;
    /*
     * Prepare a DataRow message    U表示可验证信息
     */

    pq_beginmessage(&buf, 'U');
	//by zcy modified 2020/6/30
	pq_sendint(&buf, slot->hashmap_num, 4);
	//modified end	

    pq_sendint(&buf, natts, 2);

	//by zcy modified 2020/6/30
    appendBinaryStringInfo(&buf, slot->hashmap_str+sequence_num*DEFAULTSIZE, DEFAULTSIZE);
	appendBinaryStringInfo(&buf, slot->vo_str+sequence_num*DEFAULTSIZE, DEFAULTSIZE);
	//modified end	
