 ...
 */
static Auth_Op_Expr *
_readAuthOpExpr(void)
{
    READ_LOCALS(Auth_Op_Expr);

    READ_NODE_FIELD(lexpr);
    READ_NODE_FIELD(rexpr);
    READ_INT_FIELD(authType);
...
_readAuthExpr(void)
{
    READ_LOCALS(Auth_Expr);

    READ_NODE_FIELD(lexpr);
    READ_NODE_FIELD(rexpr);
    READ_STRING_FIELD(opName);
...
#define MATCH(tokname, namelen) \
    ...
    else if (MATCH("Auth_Op_Expr", 12))    //added by gqx
        return_value = _readAuthOpExpr();
    else if (MATCH("Auth_Expr", 12))    //added by gqx
        return_value = _readAuthExpr();
	else
	{
