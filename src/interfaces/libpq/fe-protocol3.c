 ...
 */
static char* mbroot = NULL;
//by zcy modified 2020/6/30
#define DEFAULTSIZE 8192
// static char* mbt_buffer = NULL;
int mbt_count = 0;
char* vo_object[2];
//modified end	
void
		...
		else
		{
			...
			switch (id)
			{
				case 'C':		/* command complete */
                    ...
                    int i =0;
                    bool flag = false;
                    for(; i <= CMDSTATUS_LEN; i++ ){
                        if(conn->result->cmdStatus[i] == '$'){
                            i++;
                            flag = true;
                            break;
                        }
                    ...
                    if(mbroot == NULL){
                        mbroot = (char*)malloc(33);
                    }

                    if(flag == true){
                       memset(mbroot, 0, 33);
                       strncpy(mbroot, conn->result->cmdStatus + i, 33);
                    }
                ...
                case 'U':		/* VO information */
                {
					...
					int 		count;
					//by zcy modified 2020/6/30
					pqGetInt(&count, 4, conn);
					//获取列的个数
                    pqGetInt(&nfields, 2, conn);
					...
					if(mbt_count == 0)
					{
						...
						// 	mbt_buffer = NULL;
						// }
						// mbt_buffer = (char*)malloc(count*DEFAULTSIZE*2*sizeof(char));
						// MemSet(mbt_buffer, 0, count*DEFAULTSIZE*2);
						int i, j;
						 //假设接收到的是VO字符串，现在nfields的值是1
						for (i = 0; i < nfields; i++)
						{
							vo_object[i] = (char*)malloc(count*DEFAULTSIZE*sizeof(char));
						}
						MemSet(vo_object[0], 0, count*DEFAULTSIZE);
						MemSet(vo_object[1], 0, count*DEFAULTSIZE);

					}
					// memcpy(mbt_buffer+ (count - mbt_count)* DEFAULTSIZE* 2, conn->inBuffer + conn->inCursor, DEFAULTSIZE* 2);
					int i, j;
					for (i = 0; i < nfields; i++)
					{
						conn->inCursor = conn->inCursor + i * DEFAULTSIZE ;
						for(j = 0; j < DEFAULTSIZE; j++){
							vo_object[i][j+(count - mbt_count)* DEFAULTSIZE]=conn->inBuffer[conn->inCursor + j];
						}
					}
					conn->inCursor += DEFAULTSIZE;
...
//                        goto failure;
					...
					if(mbt_count <= 0)
					{
						...
						// {
						// 	free(conn->inBuffer);
						// 	conn->inBuffer = mbt_buffer;
						// }


						result->isAuthType = TRUE;
						int vosize = 3;
						result->vosize = vosize;
						//初始化列
						if (vosize > 0)
						{
							result->str = (PGresAttDesc *)
									pqResultAlloc(result, vosize * sizeof(PGresAttDesc), TRUE);
							MemSet(result->str, 0, vosize * sizeof(PGresAttDesc));
						}
						//该部分测试接收数据是否正常。
						...
						hashmap_deserialize(hashp, vo_object[0]);
						rangevo* vo =(rangevo*)malloc(sizeof(rangevo));
						rangevo_init(vo,count * DEFAULTSIZE);
						vo_deserialize(vo, vo_object[1]);
						char* root = (char*)malloc(32);
//                        startTime = GetTime();
						//reconstruct_root(tree,hashp,vo);
                        reconstruct_root(hashp, *vo, root);
                        //strcpy(root,"11111111111111111111111111111111");
//                        endTime = GetTime();
//                        mytime =mytime + ((double)(endTime - startTime)) ;
//                        count1++;
//                        if(count1==99){
//                           printf("\n\n\n\nreconstruct_root times = %f\n\n\n\n",mytime);
//                           count1=0;
//                           mytime=0;
//                        }
						result->str[0].name = root;
						result->str[1].name = (char*)malloc(32);
						strncpy(result->str[1].name, mbroot, 32);
						...
						if(flag)
							result->str[2].name = "TRUE";
						else
							result->str[2].name = "FALSE";
					}

					
                    break;
                }
                case 'L':		/* VO information */
                {
                    int			nfields, i;
                    if(mbroot == NULL){
                        mbroot = (char*)malloc(32);
                        MemSet(mbroot, 0, 32);
                    }
                    //获取列的个数
                    pqGetInt(&nfields, 2, conn);
                    ...
                    for (i = 0; i < nfields; i++)
                    {
                        ...
                        StrNCpy(mbroot, conn->workBuffer.data, 32);
//                        vo_object= pqResultStrdup(result,
