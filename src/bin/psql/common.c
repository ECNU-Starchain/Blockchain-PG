...
PrintQueryResults(PGresult *results)
{
	...
	switch (PQresultStatus(results))
	{
		case PGRES_TUPLES_OK:
            ...
            /* if it's /UPDATE/DELETE RETURNING, also print status */
...
SendQuery(const char *query)
{
	...
	if (pset.fetch_count <= 0 || !is_select_command(query))
	{
		...
		if(strncmp(query, "INSERT", 6) == 0){
            // time_t tmpcal_ptr1, tmpcal_ptr2;
            // time(&tmpcal_ptr1);
            ...
            for(; i < 1000; i++){
                MemSet(str, 0 , 100);
                snprintf(str, 80, "INSERT INTO user_tb(name, age) VALUES('l3iw124', %d);", i);

             	results = PQexec(pset.db, str);
            }
			free(str);
			finish = clock();
			endTime = GetTime();
            // time(&tmpcal_ptr2);
            double mytime = (double)(endTime - startTime) ;
			printf("\n\n\n\ninsert times = %f\n\n\n\n",mytime);
        }else{
			if(strncmp(query, "authselect", 10) == 0){
				// time_t tmpcal_ptr1, tmpcal_ptr2;
				// time(&tmpcal_ptr1);
				clock_t start, finish;
				start = clock();
                ...
                for(; i < 2; i++){
					MemSet(str, 0 , 100);
                    snprintf(str, 80, "authselect * from user_tb where age>1 and age < 100");
					results = PQexec(pset.db, str);
				}
				free(str);
				finish = clock();
			...
			else
			{
				if(strncmp(query, "select", 6) == 0){
					...
					for(; i < 100; i++){
						MemSet(str, 0 , 100);
						snprintf(str, 80, "select * from user_tb where age>1 and age < 9000");
						results = PQexec(pset.db, str);
					}
					free(str);
					finish = clock();
