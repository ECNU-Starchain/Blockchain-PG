...
print_aligned_text(const printTableContent *cont, FILE *fout)
{
    ...
    if(cont->vosize > 0){
        ...
        //30
        fputs("     ", fout);   //6
        fputs("auth range query roothash", fout);      //8
        fputs("    |", fout);  //6

        //
        fputs("        ", fout);   //6
        fputs("local mbtre roothash", fout);      //8
        fputs("      |", fout);  //6
        ...
        int j = 0;
        for(j = 0; j < cont->vosize - 1; j++){
            fputs(" ", fout);
            fputs(cont->vocells[j], fout); //23
            fputs(" |", fout);
        }

        fputs("  ", fout);
        fputs(cont->vocells[2], fout);
        fputs(" \n\n", fout);
    }
...
printTableInit(printTableContent *const content, const printTableOpt *opt,
               const char *title, const int ncolumns, const int nrows,  int vosize)
{
	content->opt = opt;
	content->title = title;
    ...
    //added by gqx
    content->vocells = pg_local_calloc(vosize + 1,
                                         sizeof(*content->vocells));
    content->vosize = 0;
...
printQuery(const PGresult *result, const printQueryOpt *opt, FILE *fout, FILE *flog)
{
	...
	int			i,
				r,
				c;
    //default value added by gqx
    int vo_size = 3;
	if (cancel_pressed)
		return;

	printTableInit(&cont, &opt->topt, opt->title,
                   PQnfields(result), PQntuples(result), vo_size);
    ...
    if(result->isAuthType && result->vosize > 0){
        cont.vosize = result->vosize;
        int j = 0;
        for(; j < result->vosize; j++){
            cont.vocells[j] = result->str[j].name;
        }
    }
